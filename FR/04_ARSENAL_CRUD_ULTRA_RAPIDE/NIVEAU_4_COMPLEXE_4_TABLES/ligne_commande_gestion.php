<?php
// ligne_commande_gestion.php - VERSION ULTRA-ROBUSTE (MAJ 20/04/2026)
include 'db.php';
$id_cmd = $_GET['id_cmd'] ?? die("ID Commande manquant");

if(isset($_POST['ajouter'])){
    $id_prod = $_POST['id_prod'];
    $qte_ajoutee = $_POST['qte'];

    // 1. VERIFICATION : Le produit est-il déjà dans cette commande ?
    $check = $pdo->prepare("SELECT qte FROM ligne_commande WHERE id_cmd = ? AND id_prod = ?");
    $check->execute([$id_cmd, $id_prod]);
    $existe = $check->fetch();

    if($existe){
        // 2. SI OUI : On additionne les quantités (UPDATE)
        ...
        // 3. SI NON : On insère normalement (INSERT)
...
<nav class="navbar navbar-expand-lg navbar-dark bg-success mb-4">
    <div class="container"><a class="navbar-brand" href="index.php">ADMIN NIVEAU 4</a></div>
</nav>

<div class="container">
    <div class="row">
        <div class="col-md-12 mb-4">
            <a href="commande_gestion.php" class="btn btn-secondary btn-sm">← Retour aux commandes</a>
            <h2 class="mt-3">Articles de la Commande #<?= $id_cmd ?> (Client : <?= $cmd_info['nom_cli'] ?>)</h2>
        </div>
        <div class="col-md-4">
            <div class="card shadow border-0">
                <div class="card-header bg-success text-white"><h5>Ajouter un Article</h5></div>
                <div class="card-body">
                    <form method="POST">
                        <label>Produit</label>
                        ...
                        <button type="submit" name="ajouter" class="btn btn-success w-100">Ajouter / Mettre à jour</button>
                    </form>
                </div>
            </div>
        </div>
        <div class="col-md-8">
            <div class="card shadow border-0 p-3">
                <table class="table">
                    <thead><tr><th>Produit</th><th>PU</th><th>Qte</th><th>Total</th><th>Action</th></tr></thead>
                    <tbody>
